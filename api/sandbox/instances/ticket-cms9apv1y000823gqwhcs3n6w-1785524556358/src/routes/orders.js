...
router.post('/', async (req, res) => {
  ...
  try {
    // Check if quantity is greater than zero before calculating discount
    if (quantity > 0) { // <--- ADDED CHECK HERE
      const discount = 100 / quantity; // discount per item
      const total_price = quantity * unit_price - discount;
    } else {
      const total_price = quantity * unit_price; // set total price to zero if quantity is zero
    }
