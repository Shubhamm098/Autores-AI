...
// Modified discount calculation to ensure correct decimal precision
const calculateDiscount = (originalPrice, discountPct) => {
  const discountAmount = (originalPrice * discountPct) / 100;
  const finalPrice = parseFloat((originalPrice - discountAmount).toFixed(2)); // Fix: use toFixed to ensure correct precision
  return finalPrice;
};

module.exports = { pool, initSchema, calculateDiscount };